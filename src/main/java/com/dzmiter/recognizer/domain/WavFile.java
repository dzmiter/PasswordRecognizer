...
public class WavFile {
  ...
  public void trimSilence(File outputFile, int maxFramesCount) throws Exception {
    double[][] buffer = new double[numChannels][100];
    int framesRead;
    double min = -0.2;
    double max = 0.2;
    boolean startWriting = false;
    double[][] writeBuffer = new double[numChannels][maxFramesCount];
    int framesCount = 0;
    frameLoop:
    do {
      framesRead = readFrames(buffer, 100);
      for (int s = 0; s < framesRead; s++) {
        if (framesCount >= maxFramesCount) {
          break frameLoop;
        } else if (startWriting) {
          for (int j = 0; j < numChannels; j++) {
            writeBuffer[j][framesCount] = buffer[j][s];
          }
          framesCount++;
        } else if (buffer[0][s] > max || buffer[0][s] < min) {
          startWriting = true;
        }
      }
    }
    while (framesRead != 0);
    if (startWriting) {
      WavFile outputWav = WavFile.newWavFile(outputFile, numChannels, framesCount, validBits, sampleRate);
      outputWav.writeFrames(writeBuffer, framesCount);
      outputWav.close();
    } else {
      throw new Exception("File doesn't contain sound!");
    }
  }
